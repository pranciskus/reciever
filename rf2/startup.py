...
from rf2.util import set_slider_value, set_updown_value, select_from_list, set_window_elements_value
...
def oneclick_start_server(server_config: dict) -> bool:
    ...
    server_binary_path = join(
        server_root_path, "Bin64", "rFactor2 Dedicated.exe")
    server_binary_commandline = server_binary_path + \
    ...
    max_clients_overwrite = str(server_config["mod"]["server"]["overwrites"]
                                ["Multiplayer.JSON"]["Multiplayer Server Options"]["Max MP Players"])

    if exists(mod_ini_path):
        remove(mod_ini_path)
    with open(mod_ini_path, "w") as file:
        file.write("[SETTINGS]\n")
        file.write("MaxClients=" + max_clients_overwrite + "\n")
        file.write("[TRACKS]\n")
...
def stop_server(server_config: dict) -> bool:
    ...
    binary_path = join(root_path, "Bin64",
...
def select_mod_and_admin_password(window: WindowSpecification, admin_password: str, mod_name: str, mod_version: str):
    """
...
def select_grip(start_window: WindowSpecification, grips: dict, tracks: dict, server_root_path: str, rf2_server: Application):
    """
    Select the mod and inserts the admin password
    ...
    for _, track in tracks.items():
        layout = track["layout"]
        event_list = start_window.window(
            best_match='Selected Events:ListBox')
